...
def call_periodic(delay, interval, task):
    'Starting at time()+delay, run the task repeatedly at fixed intervals'
    def inner():
        task()
...
def event_loop():
    while events:
        while events[0].event_time > time():  # if next event is not due yet
            sleep(0.1)
        event = heappop(events)  # O(log n)       
        event.task()
...
def show_five_three_times():
    call_later(13, partial(show_msg,'Five'))
    call_later(23, partial(show_msg,'Five'))
    call_later(28, partial(show_msg,'Five'))

call_periodic(13, 2, partial(show_msg, 'Heartbeat!'))  #infinite loop
call_later(10, show_two)
call_later(5, show_ten)
call_later(20, show_two)
call_later(1, partial(show_msg,'Howdy'))
call_later(35, partial(show_msg, 'C U'))
