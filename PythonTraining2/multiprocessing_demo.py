...
if IO_BOUND: # Using separate threas whn you are IO bound
    from multiprocessing.pool import ThreadPool as Pool
else: # Use seperate processes when  you are CPU bound
...
def sitesize(site):
    ''' Determine the size of the website

    This is non-parallizeable
    1) DNS lookup(UDP request/response)
    2) Acquire a socket from the OS
    3) TCP connection (SYN, ACK, SYN/ACK)
    4) Send HTTP GET request for the root resourse
    5) Wait for the TCP response which is broken up into maay packages that need to be reassembled
    6) Count the number of characters on the pge. 
    '''
